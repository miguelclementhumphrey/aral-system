...
const setPasswordSchema = z.object({
  schoolId: z.string().min(1, "School context is required"),
  tempCredential: z.string().optional(),
...
export default function SetPassword() {
  ...
  const defaultSchoolId = useMemo(() => {
    const tempToken = sessionStorage.getItem("aral_token");
    if (!tempToken) return "";

    try {
      const decoded: { schoolId?: string } = jwtDecode(tempToken);
            ...
            <Form {...form}>
              <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
                ...
                <Button 
                  ...
                  size="lg"
                  disabled={setPasswordMutation.isPending || !schoolId}
                >
